...
function check(elementData) {
  ...
  if (elementData.onBeginLeave || elementData.onCompleteEnter) {
    ...
    var isWithinTop = viewport.top <= elementData.top;
    var isWithinRight = viewport.right >= elementData.right;
    var isWithinBottom = viewport.bottom >= elementData.bottom;
    var isWithinLeft = viewport.left <= elementData.left;
